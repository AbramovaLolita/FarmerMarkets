...
def markets_list_db(limit,offset,sort_by,order):
    conn = get_connection()
    cur = conn.cursor()
    query = '''select m.name, 
...
def market_search_db(city=None,state=None,zip_code=None,sort_by=None,order=None, user_lat=None, user_long=None,max_dist=None):
    conn = get_connection()
    cur = conn.cursor()
